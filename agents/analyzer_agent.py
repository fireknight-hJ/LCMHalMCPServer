...
async def analyze_functions(function_list):
    ...
    async def _classify_with_retry(fn: str):
        for attempt in range(max_retries):
            try:
                result = await function_classify(fn)
                if result is not None:
                    return result
                print(
                    f"Warning: Empty result for {fn}, retrying "
                    f"({attempt + 1}/{max_retries})..."
                )
            except Exception as exc:
                print(
                    f"Error analyzing {fn} "
    ...
    if max_c is None:
        ...
        async def _run(fn: str):
            return await _classify_with_retry(fn)
