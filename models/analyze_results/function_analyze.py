...
class FunctionClassifyResponse(BaseModel):
    ...
    function_type: Literal["CORE", "RECV", "IRQ", "RETURNOK", "SKIP", "NODRIVER", "INIT", "LOOP"] = Field(default="NODRIVER", description="CORE = NVIC/OS kernel/VTOR critical (no replacement); SysTick is not CORE. Other types per classification rules.")
    ...
    replacement_check_reason: Optional[str] = Field(default=None, description="If rubric check failed on function_replacement, reason for failure (for retry or logging)")
