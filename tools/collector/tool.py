...
async def register_db_tool(db_path: str):
    """注册数据库路径"""
...
async def validate_database_tool(db_path: str):
    """验证数据库路径和完整性"""
...
async def get_files_in_db_zip_tool(db_path: str):
    """获取数据库zip文件中的文件列表"""
...
async def get_tree_in_db_zip_tool(db_path: str):
    """获取数据库zip文件中的目录树"""
...
async def get_mmio_func_list_tool(db_path: str):
    """获取MMIO函数列表"""
...
async def get_mmio_files_tool(db_path: str):
    """获取MMIO相关文件列表"""
...
async def get_mmio_func_info_tool(db_path: str, func_name: str):
    """获取MMIO函数详细信息"""
...
async def get_function_info_tool(db_path: str, func_name: str):
    """获取通用函数信息"""
...
async def get_struct_or_enum_info_tool(db_path: str, struct_name: str):
    """获取结构体或枚举信息"""
...
async def get_func_call_stack_tool(db_path: str, func_name: str, layer_cnt: int = 1):
    """获取函数调用栈"""
...
async def get_driver_info_tool(db_path: str, driver_name: str):
    """获取驱动信息"""
