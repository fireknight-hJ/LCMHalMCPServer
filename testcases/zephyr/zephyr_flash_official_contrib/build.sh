...
# 检查构建结果
ELF_FILE="$BUILDDIR/zephyr_flash_test"
if [ -f "$ELF_FILE" ]; then
    echo "========================================"
    echo "Zephyr官方Flash驱动测试构建成功!"
    echo "========================================"
    echo "输出文件: $ELF_FILE"
    ...
    # 显示符号信息
    echo ""
    echo "生成的函数符号 (Flash相关):"
    if command -v arm-none-eabi-nm >/dev/null 2>&1; then
        arm-none-eabi-nm "$ELF_FILE" | grep -E "(flash_|test_|main|HAL_BE)" | head -20
    else
        nm "$ELF_FILE" | grep -E "(flash_|test_|main|HAL_BE)" | head -20
    fi
...
# 测试配置
test_cases:
  - name: "flash_erase_single_page_test"
    description: "Flash单页擦除测试"
  - name: "flash_program_test"
    description: "Flash编程测试"
  - name: "flash_mass_erase_test"
    description: "Flash批量擦除测试"
  - name: "flash_error_handling_test"
    description: "Flash错误处理测试"
...
EOF
    ...
    # 创建简单的输入文件
    echo "创建测试输入文件..."
    echo "FLASH_TEST" > "$SCRIPTDIR/emulate/input.txt"
    echo -n "FLASH" > "$SCRIPTDIR/emulate/base_input/input.bin" 2>/dev/null || mkdir -p "$SCRIPTDIR/emulate/base_input" && echo -n "FLASH" > "$SCRIPTDIR/emulate/base_input/input.bin"
    
    echo "========================================"
    echo "Flash驱动测试用例准备完成!"
