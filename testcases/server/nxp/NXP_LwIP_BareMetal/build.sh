...
export APP_PATH="mcuxsdk/examples/lwip_examples/lwip_httpsrv/bm"
export BUILD_DIR="$PWDDIR/build-eth"
./build_firmware.sh
...
if [ $? -eq 0 ]; then
    ...
    mv -f "build-eth/lwip_httpsrv_bm.elf" "$SCRIPTDIR/emulate/output.elf" || {
