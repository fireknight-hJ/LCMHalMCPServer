...
export APP_PATH="mcuxsdk/examples/fatfs_examples/fatfs_ramdisk_freertos"
export BUILD_DIR="$PWDDIR/build-fatfs-freertos"
./build_firmware.sh
...
if [ $? -eq 0 ]; then
    ...
    mv -f "build-fatfs-freertos/fatfs_ramdisk_freertos.elf" "$SCRIPTDIR/emulate/output.elf" || {
