...
export APP_PATH="mcuxsdk/examples/freertos_driver_examples/freertos_lpuart"
export BUILD_DIR="$PWDDIR/build-uart-freertos"
./build_firmware.sh
...
if [ $? -eq 0 ]; then
    ...
    mv -f "build-uart-freertos/freertos_lpuart.elf" "$SCRIPTDIR/emulate/output.elf" || {
