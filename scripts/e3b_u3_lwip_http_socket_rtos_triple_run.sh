#!/usr/bin/env bash
# E3b U3：STM32Cube「LwIP HTTP Server Socket RTOS」三连跑（独立 LLM session + 可归档 isolated_db）
#
# 仓库 testcase：testcases/server/stm32/LwIP_HTTP_Server_Socket_RTOS
# （口头/文档中的 LwIP_HTTP_RTOS_Socket_Server 与此同指。）
#
# 用法（仓库根目录）：
#   bash scripts/e3b_u3_lwip_http_socket_rtos_triple_run.sh
#
# 依赖：
...
if [[ "${E3B_U3_CONFIRM:-}" != "yes" ]]; then
  ...
  echo "  bash scripts/e3b_u3_lwip_http_socket_rtos_triple_run.sh" >&2
  exit 2
fi

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TC="${TC:-testcases/server/stm32/LwIP_HTTP_Server_Socket_RTOS}"
DB="${DB:-/home/haojie/workspace/dbs_server/DATABASE_LwIP_HTTP_Server_Socket_RTOS}"
PROJ="${PROJ:-/home/haojie/posixGen_Demos/LwIP_HTTP_Server_Socket_RTOS}"
PYTHON="${PYTHON:-python3}"

STAMP="$(date +%Y%m%d_%H%M%S)"
BATCH="${BATCH:-$ROOT/runs/e3b_uncertainty/${STAMP}_U3_LwIP_HTTP_Server_Socket_RTOS}"
BASELINE="$BATCH/db_baseline_oem"

mkdir -p "$BATCH/logs" "$BATCH/isolated_db" "$BATCH/artifacts"

echo "[e3b-u3] BATCH=$BATCH"
...
# 等同 rsync -a --delete：环境无 rsync 时用 cp（Docker/精简镜像常见）
mirror_replace() {
  local src="$1" dst="$2"
  [[ -d "$src" ]] || {
    echo "[e3b-u3] mirror_replace: 源目录不存在: $src" >&2
    return 1
  }
  rm -rf "$dst"
  mkdir -p "$dst"
...
echo "[e3b-u3] 清空首轮 ai_log 并制作 OEM 快照（CodeQL DB + src.zip 等）…"
"$PYTHON" main.py clean "$TC" --all 2>/dev/null || true
mkdir -p "$(dirname "$BASELINE")"
mirror_replace "$DB" "$BASELINE"

restore_oem() {
  echo "[e3b-u3] 从快照恢复 DB → $DB"
  mirror_replace "$BASELINE" "$DB"
  echo "[e3b-u3] git reset 工程: $PROJ"
...
for i in 1 2 3; do
  ...
  echo "[e3b-u3] main_exit_code=$ec" | tee -a "$BATCH/logs/U3_run${i}.log"
...
echo "    --summary-tsv doc/experiments/E3b-U3_LwIP_HTTP_Server_Socket_RTOS_${STAMP}_实测汇总.tsv"
echo ""
