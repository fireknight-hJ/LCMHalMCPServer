...
#   scripts/batch_main_analyze_stm32_parallel.sh 6
#   scripts/batch_main_analyze_stm32_parallel.sh 4 testcases/server/stm32/UART_Hyperterminal_IT
#   STM32_ROOT=testcases/server/stm32 scripts/batch_main_analyze_stm32_parallel.sh 8
#   MAIN_CMD=build scripts/batch_main_analyze_stm32_parallel.sh 4   # 先 build 再汇总
#   MAIN_CMD= scripts/batch_main_analyze_stm32_parallel.sh 4        # 跳过主流程，仅汇总
...
ONLY_DEMO="${2:-}"
STM32_ROOT="${STM32_ROOT:-testcases/server/stm32}"
LOG_DIR="${LOG_DIR:-run/batch_main_analyze_stm32_logs}"
...
if [[ -n "$ONLY_DEMO" ]]; then
  ...
  mapfile -t DEMOS < <(find "$STM32_ROOT" -type f -name "lcmhal_config.yml" | sed 's#/lcmhal_config.yml$##' | sort)
fi

if [[ "${#DEMOS[@]}" -eq 0 ]]; then
  echo "未找到 demo：$STM32_ROOT 下没有 lcmhal_config.yml"
  exit 1
fi

echo "ROOT_DIR=$ROOT_DIR"
echo "STM32_ROOT=$STM32_ROOT"
