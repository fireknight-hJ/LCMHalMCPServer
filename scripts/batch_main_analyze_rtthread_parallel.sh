...
#   scripts/batch_main_analyze_rtthread_parallel.sh 6
#   scripts/batch_main_analyze_rtthread_parallel.sh 4 testcases/server/rtthread/imxrt1052-nxp-evk/i2c
#   RTTHREAD_ROOT=testcases/server/rtthread scripts/batch_main_analyze_rtthread_parallel.sh 8
#   MAIN_CMD=build scripts/batch_main_analyze_rtthread_parallel.sh 4   # 先 build 再汇总
#   MAIN_CMD= scripts/batch_main_analyze_rtthread_parallel.sh 4        # 跳过主流程，仅汇总
...
ONLY_DEMO="${2:-}"
RTTHREAD_ROOT="${RTTHREAD_ROOT:-testcases/server/rtthread}"
LOG_DIR="${LOG_DIR:-run/batch_main_analyze_rtthread_logs}"
...
if [[ -n "$ONLY_DEMO" ]]; then
  ...
  mapfile -t DEMOS < <(find "$RTTHREAD_ROOT" -type f -name "lcmhal_config.yml" | sed 's#/lcmhal_config.yml$##' | sort)
fi

if [[ "${#DEMOS[@]}" -eq 0 ]]; then
  echo "未找到 demo：$RTTHREAD_ROOT 下没有 lcmhal_config.yml"
  exit 1
fi
