...
#   scripts/batch_main_analyze_nxp_parallel.sh 6
#   scripts/batch_main_analyze_nxp_parallel.sh 4 testcases/server/nxp/NXP_I2C_BareMetal
#   NXP_ROOT=testcases/server/nxp scripts/batch_main_analyze_nxp_parallel.sh 8
#   MAIN_CMD=build scripts/batch_main_analyze_nxp_parallel.sh 4   # 先 build 再汇总
#   MAIN_CMD= scripts/batch_main_analyze_nxp_parallel.sh 4        # 跳过主流程，仅汇总
...
ONLY_DEMO="${2:-}"
NXP_ROOT="${NXP_ROOT:-testcases/server/nxp}"
LOG_DIR="${LOG_DIR:-run/batch_main_analyze_nxp_logs}"
...
if [[ -n "$ONLY_DEMO" ]]; then
  ...
  mapfile -t DEMOS < <(find "$NXP_ROOT" -type f -name "lcmhal_config.yml" | sed 's#/lcmhal_config.yml$##' | sort)
fi

if [[ "${#DEMOS[@]}" -eq 0 ]]; then
  echo "未找到 demo：$NXP_ROOT 下没有 lcmhal_config.yml"
  exit 1
fi

echo "ROOT_DIR=$ROOT_DIR"
echo "NXP_ROOT=$NXP_ROOT"
