...
**可用工具：** BuildProject（执行编译，返回 std_out、std_err、exit_code）；GetReplaceFuncDetailsByFile（按文件路径查看替换）；GetFunctionAnalysisAndReplacement / GetFunctionAnalysisAndReplacementFormatted（按函数名查看分析与替换，Formatted 更易读）；UpdateFunctionReplacement（提交替换：function_name, replace_code, reason）；**FixFunctionBuildErrors**（子 agent：针对单个函数修复编译错误，入参为 function_name 与对应的 error_info 即 stderr 片段；错误较多时务必使用，避免上下文过长）。

在编译过程中，可能会遇到一些错误，这些错误可能是替换函数过程中引入的问题，你需要不断根据错误信息反馈修改程序源码，确保项目能够成功编译。

**当 UpdateFunctionReplacement 返回 ok: false 时：**（1）若为 rubric 校验失败，会返回 reason（如要求保留 NVIC/SysTick 等写操作），需按 reason 修改替换代码后再次调用。（2）若为编译验证失败，会返回 reason 为 "Compile verification failed for replacement." 且带 **build_stderr**（单函数替换后的编译 stderr），需根据 build_stderr 修正语法/类型错误后再次调用。同一函数最多重试 2–3 次；若仍失败可调用 **FixFunctionBuildErrors(function_name, build_stderr)** 委托子 agent。
...
**Available tools:** BuildProject (run build; returns std_out, std_err, exit_code); GetReplaceFuncDetailsByFile (replacements by file path); GetFunctionAnalysisAndReplacement / GetFunctionAnalysisAndReplacementFormatted (analysis and replacement by function name; Formatted is easier for LLM); UpdateFunctionReplacement (apply a replacement: function_name, replace_code, reason); **FixFunctionBuildErrors** (sub-agent: fix build errors for a single function — input function_name and exact error_info stderr snippet; use when there are many errors to avoid context overflow).
...
**When UpdateFunctionReplacement returns ok: false:** The tool may return two kinds of failure. (1) **Rubric failure:** it returns a `reason` (e.g. "must preserve NVIC/SysTick/VTOR writes"). Fix the replacement according to that reason and call UpdateFunctionReplacement again; do not treat as success. (2) **Compile verification failure:** it returns `reason: "Compile verification failed for replacement."` and **build_stderr** (the compiler stderr from a single-function replacement build). Use build_stderr to fix syntax/type errors in your replacement, then call UpdateFunctionReplacement again. Limit retries per function to 2–3; if still failing after that, call **FixFunctionBuildErrors(function_name, build_stderr)** to delegate to the sub-agent.
