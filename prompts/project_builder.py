...
When stderr has **many** errors or many functions to fix, prefer to delegate: for each error (or group of errors) that you attribute to a **single function**, call **FixFunctionBuildErrors** with `function_name` and `error_info` (the relevant stderr lines). This keeps the main conversation focused and avoids context overflow or hitting the 50-step limit. After FixFunctionBuildErrors returns, you can call BuildProject again to see the overall state; if more errors remain, either call FixFunctionBuildErrors for another function or fix directly as before.
